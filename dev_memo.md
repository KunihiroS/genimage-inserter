...
### 画像挿入仕様
- 画像リンクは**Markdown形式**（`![](path/to/image.png)`）で挿入する。
  - 理由: GitHub等の外部サービスでも表示可能な汎用性を確保するため
- 挿入位置:
  - テキスト選択時: 選択範囲の末尾に改行して挿入
  - 全文対象時: ノート末尾に改行して挿入

### 処理中の通知
- 設定で指定した秒数（デフォルト3秒）経過後、「画像生成中...」の通知を表示する。
- 処理完了時に通知を閉じる。

### プロンプトファイル仕様
- 設定で指定した**Vault内ディレクトリ**の `.md` ファイルをプロンプトファイルとして認識する。
...
- プロンプト選択モーダルには**ファイル名**（拡張子なし）が表示される。
- プロンプトディレクトリが空または未設定の場合は**エラー通知**して終了する。
...
| 設定項目 | 説明 | 必須 | デフォルト |
|----------|------|:----:|-----------|
| `.env` のPATH | API Key等を記載した`.env`ファイルのパス | ✓ | - |
| プロンプトディレクトリ | プロンプトファイル（`.md`）を格納するVault内ディレクトリ | ✓ | - |
| 画像保存先ディレクトリ | 生成画像を保存するVault内ディレクトリ | ✓ | - |
| 処理中通知の表示秒数 | 「画像生成中...」通知を表示するまでの秒数（0で即時表示） | | `3` |

### 設定項目の補足
- `.env` のPATH: **Vault外**であることが必須（セキュリティのため）
- プロンプトディレクトリ: **Vault内**に配置（Obsidianで編集しやすく、Git同期も容易）
- 画像保存先ディレクトリ: 指定ディレクトリ配下に `{notefilename}/` フォルダが作成され、その中に画像が保存される
  - 例: 設定が `assets/generated`、ノート名が `my-note` の場合 → `assets/generated/my-note/20260207123456_my-note.png`
